...
public class InteractiveAction extends BaseAction {
	...
	public void releaseInteractive() throws IOException {
		...
		if (isMultipart) {
			...
			if (files.length > 0 && fileNames.length > 0) {
				for (int i = 0; i < files.length; i++) {
					ext = fileNames[i].substring(fileNames[i].lastIndexOf("."),
							fileNames[i].length());
					String fileName = sdf.format(nowDate) + "_"
					...
					FileUtils.copyFile(files[i],
							new File(path + dir + fileName));

					File file = new File(path + thumbnailDir + dir);
					if (!file.exists()) {
						file.mkdir();
					}
					ThumbnailUtil ccc = new ThumbnailUtil(
							path + dir + fileName, path + thumbnailDir + dir
									+ fileName);
					ccc.resize(Integer.parseInt(resize),
							Integer.parseInt(resize));

					Picture picture = new Picture();
					picture.setUrl(serverUrl + dir + fileName);
					picture.setThumbnailUrl(serverUrl + thumbnailDir + dir
							+ fileName);
		 ...
		 * List<String> accountList = new ArrayList<String>(); List<AcUser>
		 * userList = accountService.findUser(" and communityId = " +
		 * communityId); if(userList.size()>0){ for(AcUser user:userList){
		 * String account = user.getDeviceAccount(); accountList.add(account); }
	...
	public void getInteractiveList() throws IOException {
		...
		if (user != null) {
			...
			;
			in = in.replace(",", "|");
			String notIn = valueString.substring(valueString.indexOf("|") + 1,
					valueString.length());
			notIn = notIn.replace(",", "|");

			HashMap<String, Object> rethm = interactiveService
					.seachInteractive(in, notIn, Integer.parseInt(page), rows);
			...
			if (list != null && !list.isEmpty()) {
				...
				while (iterator.hasNext()) {
					...
					json.put("id", id);
					json.put("subject",
							String.valueOf(obj[1] == null ? "" : obj[1]));
					json.put("createTime",
							String.valueOf(obj[2] == null ? "" : obj[2]));
					String uId = String.valueOf(obj[3] == null ? "" : obj[3]);
					String picIds = String
							.valueOf(obj[4] == null ? "" : obj[4]);
					if (picIds != null && !picIds.equals("")) {
						String[] ids = picIds.split(",");
						if (ids.length > 0) {
							Picture picture = pictureService.loadPicture(Long
									.parseLong(ids[0]));
							if (picture != null) {
					...
					if (id != null && !id.equals("")) {
						int praiseNum = replyService
								.countPraise(" and releaseId = "
										+ Long.parseLong(id)
										+ " and releaseType = '"
										+ DictionaryUtil.RELEASE_TYPE_02 + "'");
						int commentNum = replyService
								.countComment(" and releaseId = "
										+ Long.parseLong(id)
										+ " and releaseType = '"
										+ DictionaryUtil.RELEASE_TYPE_02 + "'");
	...
	public String getHobbies(Long userId) {
		...
		if (userDetail != null) {
			if (userDetail.getCatering() != null
					&& !userDetail.getCatering().equals("")) {
				valueString += "," + userDetail.getCatering();
			}
			if (userDetail.getSocialContact() != null
					&& !userDetail.getSocialContact().equals("")) {
				valueString += "," + userDetail.getSocialContact();
			}
			if (userDetail.getTravel() != null
					&& !userDetail.getTravel().equals("")) {
				valueString += "," + userDetail.getTravel();
			}
			if (userDetail.getSports() != null
					&& !userDetail.getSports().equals("")) {
				valueString += "," + userDetail.getSports();
			}
			if (userDetail.getMusic() != null
					&& !userDetail.getMusic().equals("")) {
				valueString += "," + userDetail.getMusic();
			}
			if (userDetail.getOthers() != null
					&& !userDetail.getOthers().equals("")) {
				valueString += "," + userDetail.getOthers();
			}
			if (userDetail.getCommunityNeed() != null
					&& !userDetail.getCommunityNeed().equals("")) {
				valueString += "," + userDetail.getCommunityNeed();
			}
			if (userDetail.getDislike() != null
					&& !userDetail.getDislike().equals("")) {
	...
	public void getInteractiveDetail() throws IOException {
		...
		PubInteractive interactive = interactiveService.loadInteractive(Long
				.parseLong(id));
		if (interactive != null) {
			json = JSONObject.fromObject(interactive);
			String startTime = DateFormatUtil.dateToStringM(interactive
					.getStartTime());
	...
	public void doPraise() throws IOException {
		...
		List<PubPraise> list = replyService.findPraise(" and releaseId = " + id
				+ " and releaseType = '" + DictionaryUtil.RELEASE_TYPE_02
				+ "' and userId = " + userId);
		if (list.size() > 0) { // 判断是否已赞过..
			...
			int num = replyService.countPraise(" and releaseId = "
					+ Long.parseLong(id) + " and releaseType = '"
					+ DictionaryUtil.RELEASE_TYPE_02 + "'");
			jsonObject.put("result", "1");
			jsonObject.put("value", "SUCCESS!");
			...
			PubInteractive interactive = interactiveService
					.loadInteractive(Long.parseLong(id));
			if (interactive != null) {
				Long uId = interactive.getUserId();
				AcUser u = accountService.loadUser(uId);
				if (u != null) {
					AcUser user = accountService.loadUser(Long
							.parseLong(userId));
					if (user != null) {
						PushIOS.pushSingleDevice(user.getNickName() + "赞了你的信息",
								u.getDeviceToken()); // 通知个人用户有人点赞..
	...
	public void doComment() throws IOException {
		...
		int num = replyService.countComment(" and releaseId = "
				+ Long.parseLong(id) + " and releaseType = '"
				+ DictionaryUtil.RELEASE_TYPE_02 + "'");
		jsonObject.put("result", "1");
		jsonObject.put("value", "SUCCESS!");
		...
		if (interactive != null) {
			...
			if (u != null) {
				AcUser user = accountService.loadUser(Long.parseLong(userId));
				if (user != null) {
					PushIOS.pushSingleDevice(user.getNickName() + "评论了你的信息",
							u.getDeviceToken()); // 通知个人用户有人评论
				}
			}
		}
	...
	public void listComment() throws IOException {
		...
		List<PubComments> list = replyService.findComments(" and releaseId = "
				+ id + " and releaseType = '" + DictionaryUtil.RELEASE_TYPE_02
				+ "' order by createTime desc ", Integer.parseInt(page), rows);
		if (list.size() > 0) {
