...
public class JsonUtil {
	...
	/**  2
	 * 将java对象转换成json字符串(包含日期) 
	 * @param bean  
	 * @return  
	 */
	public static String beanToJsonOfDate(Object bean,String formaterType) {
		
		JsonConfig config = new JsonConfig();
		config.registerJsonValueProcessor(java.util.Date.class, new DateJsonValueProcessor(formaterType));
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static String beanListToJsonOfDate(List beans,String formaterType) {
		
		StringBuffer rest = new StringBuffer();
		rest.append("[");
		int size = beans.size();
		for (int i = 0; i < size; i++) {
			rest.append(beanToJsonOfDate(beans.get(i),formaterType)+((i<size-1)?",":""));
		}
		rest.append("]");
	...
	/**  
	 * 从json HASH表达式中获取map，该map支持嵌套功能  
	 * @param jsonString  
	 * @return  
	 */
